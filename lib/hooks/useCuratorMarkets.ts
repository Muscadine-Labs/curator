...
import { DASHBOARD_QUERY_OPTIONS } from '@/lib/data/query-config';
...
async function fetchCuratorMarkets(chainId: number): Promise<CuratorMarketsResponse> {
  const res = await apiFetch(`/api/curator/markets?chainId=${chainId}`);
  if (!res.ok) {
...
async function fetchCuratorMarketDetail(
  ...
  const res = await apiFetch(
    `/api/curator/markets/${encodeURIComponent(marketId)}?chainId=${chainId}`
  );
  if (!res.ok) {
    const json = await res.json().catch(() => ({}));
