...
import { VaultV1Pending } from '@/components/morpho/VaultV1Pending';
...
export default function VaultDetailPage() {
  ...
  const { vault, roles, caps, queues, marketRisk, pending, vaultIsLoading, vaultIsError, vaultError } =
    useVaultV1Complete(vaultAddress);
  ...
  if (vaultIsError || !vault) {
    return (
      <AppShell
        title="Error loading vault"
        description={vaultError instanceof Error ? vaultError.message : 'Failed to load vault data'}
      >
        <Card>
          <CardHeader>
          ...
          <CardContent className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-red-600 dark:text-red-400">
              {vaultError instanceof Error ? vaultError.message : 'Failed to load vault data'}
            </p>
        ...
        <Tabs defaultValue="overview" className="space-y-3">
          ...
          <TabsContent value="overview" className="space-y-4">
            <VaultOverviewPanel
              ...
              vaultAsset={vault.asset ?? 'UNKNOWN'}
            />
